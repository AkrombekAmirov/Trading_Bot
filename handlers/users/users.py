...
async def answer_name(message: Message, state: FSMContext, raw_state: FSMContext):
    ...
    tp_20 = sorted_numbers[:custom_round(len(sorted_numbers) * 0.20)]
    tp_25 = sorted_numbers[:custom_round(len(sorted_numbers) * 0.25)]
    tp_30 = sorted_numbers[:custom_round(len(sorted_numbers) * 0.30)]
    await message.answer(
        f"Topilgan qiymatlar  80 foizi={custom_round(len(sorted_numbers) * 0.20)}\n{sorted_numbers[:custom_round(len(sorted_numbers) * 0.20)]}\n"
        f"Tegishli qiymatlar: {await extract_sp_values(tp_numbers, sp_numbers, tp_20)}")
    await message.answer(
        f"Topilgan qiymatlar  75 foizi={custom_round(len(sorted_numbers) * 0.25)}\n{sorted_numbers[:custom_round(len(sorted_numbers) * 0.25)]}\n"
        f"Tegishli qiymatlar: {await extract_sp_values(tp_numbers, sp_numbers, tp_25)}")
    await message.answer(
